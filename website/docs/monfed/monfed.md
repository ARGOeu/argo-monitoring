...
**Returns**: An array of detailed objects regarding checks that identified problematic states. Each item provides the timestamp of the check, the severity status (e.g., CRITICAL or WARNING), the target host, and the specific service type instance that was evaluated. Additionally, each object includes a detailed log message generated by the check.
